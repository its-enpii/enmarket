/**
 * Skeleton untuk halaman edit catatan.
 * Override parent (admin/posts/loading.tsx) yang menampilkan table — bukan form.
 *
 * Edit page = header (with post title) + quick-info card (status badge +
 * slug + published date + back link) + Card surface with PostForm.
 */
export default function EditPostLoading() {
  return (
    <div className="p-6 sm:p-8 space-y-6 animate-pulse">
      ...
      {/* Quick info card — status badge + slug + publish date + back link */}
      <div className="bg-surface border-2 border-ink p-4 flex flex-wrap items-center gap-3 shadow-[3px_3px_0_0_var(--color-ink)]">
        ...
        <div className="h-4 w-32 bg-ink/10" />
        <div className="h-4 w-32 bg-ink/15 ml-auto" />
      </div>
